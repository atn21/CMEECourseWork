...
rm(list=ls())
test_username <- "atn21" # this should be your username
year <- 2021
...
load_student_work <- function(student_username) {
  student_file <- paste(student_username,"_HPC_",year,"_main.R",sep="")
  load_test_data <- list()
  ...
  if(file_test("-f",student_file)) {
    ...
    load_test_data[[length(load_test_data)+1]] <-  test_data_updated(name,"name","tlc")
    load_test_data[[length(load_test_data)+1]] <-  test_data_updated(preferred_name,"preferred_name","hhi")
    load_test_data[[length(load_test_data)+1]] <-  test_data_updated(email,"email","hhh")
